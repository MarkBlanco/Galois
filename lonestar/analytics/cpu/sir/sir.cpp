...
static const char* name = "Information Propagation: S-I-R";

static const char* desc =
    "Find the nodes in the Giant Component after Infection stagnates ";
...
template <bool CONCURRENT, typename T, typename P, typename R>
void syncAlgo(Graph& graph, const P& pushWrap,
              const R& edgeRange) {
  ...
  //push all the infected nodes into the frontier
  ...
  if (CONCURRENT) {
    galois::do_all(galois::iterate(graph),
                   [&](const GNode& src) {
                     ...
                     if(status == 1){
                      pushWrap(*next, src, "parallel");
                    }
                   },
                   galois::steal(), galois::chunk_size<CHUNK_SIZE>(),
                   galois::no_stats(), galois::loopname("initFrontierParallel"));
    ...
    galois::do_all(galois::iterate(graph),
                   ...
                   galois::steal(), galois::chunk_size<CHUNK_SIZE>(),
                   galois::no_stats(), galois::loopname("initFrontier"));
  }

  assert(!next->empty());

  int i = 0;
  while (!next->empty() && i < MAX_ITERATIONS) {
    ...
    loop(galois::iterate(*curr),
         [&](const T& item) {
           ...
           for (auto e : edgeRange(item)) {
             ...
             // if the node is susceptible, try to infect it
             if (dstData == 0) {
               float infect = 1.0*(rand()/(float)RAND_MAX) ;
               if(infect <= P_sick){
                dstData = 1;
                pushWrap(*next, dst);
               }
           ...
           // if the current not is not healed, push it into the next frontier
           float heal = 1.0*(rand()/(float)RAND_MAX) ;
           if(heal <= P_heal){
             auto& srcData = graph.getData(item, flag);
             srcData = 2;
           }
           else{
             pushWrap(*next, item);
  ...
  std::cout<<"Total iter:"<<i<<std::endl;
  delete curr;
  delete next;
}
...
void runAlgo(Graph& graph) {
  ...
  //   break;
  // case Sync:
    syncAlgo<CONCURRENT, GNode>(graph, NodePushWrap(),
                                OutEdgeRangeFn{graph});
  //   break;
  // case Sync2pTile:
...
int main(int argc, char** argv) {
  ...
  galois::reportPageAlloc("MeminfoPre");
	...
	for (uint32_t i = 0; i < 16; i++){
		...
		galois::do_all(galois::iterate(graph),
				[&graph](GNode n) { graph.getData(n) = 0; });
...
//	printf("SIR Time: %f CYCLES CONVERT THIS\n", (double)t_time); //<<std::endl;
  ...
  std::cout<< accum.reduce()<<" nodes were never infected\n";
